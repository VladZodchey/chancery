...
class API:
    ...
    def create(self, user_id):
        ...
        try:
            if not self.authorized or self.db.is_permitted(user_id, WRITE):
                return Response(
                    self.db.insert_paste(content=content, uid=user_id, meta=data),
    ...
    @require_auth
    def me(self, user_id):
        """Returns information about the user hitting the request"""
        try:
            username, privileges, created_at = self.db.query(
                "SELECT username, privileges, created_at FROM users WHERE user_id = ?",
                (user_id,)
            )[0]
        except IndexError:
            self.logger.warning("Access token of a nonexistent user was used!")
            abort(HTTPStatus.INTERNAL_SERVER_ERROR, description="Unexpected error happened")
...
def register_endpoints(app, db):
    ...
    if app.config["AUTHORIZED"]:
        ...
        api_bp.add_url_rule("/me", view_func=api.me, methods=["GET"])
    if app.config["DEBUG"]:
